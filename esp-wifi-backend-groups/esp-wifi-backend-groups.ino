...
#define SENSOR_PIN 12  // GPIO pin for your sensor or button (changed from 2)
...
void sendDataViaHTTP() {
  ...
  if (!initialStatusSent) {
    ...
    bool shouldSend = (currentSensorState != lastSentSensorState) || (millis() - lastForcedSend > 30000);
...
void pollGroupStatus() {
  ...
  if (useHTTPS) {
    ...
    url = "http://" + serverIP + ":" + String(serverPort) + "/api/groups/" + groupId + "/status?excludeDevice=" + deviceId;
    http.begin(url);
  }
